...
// //                 {user.rank === 2 && <Crown className="text-gray-500 w-5 h-5" />}
// //                 {user.rank === 3 && <Crown className="text-orange-400 w-5 h-5" />}
// //                 <img 
// //                   src={user.image} 
// //                   alt={user.name} 
...
//         }
//       });
//   }, [leaderboardData, sortCriteria, skillFilter, industryFilter]);
...
//         >
//           <option value="">All</option>
//           {[...new Set(leaderboardData.map(user => user.industry))].map(industry => (
    ...
    const fetchLeaderboard = async () => {
      try {
        ...
        const transformedData = data.users.map((user, index) => ({
          ...
          progressToNextRank: user.progressToNextRank || 0
        })).filter(user => user.name !== 'Unknown'); // Remove placeholder users
